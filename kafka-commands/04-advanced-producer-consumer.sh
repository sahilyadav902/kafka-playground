#!/bin/bash
# =============================================================================
# KAFKA ADVANCED PRODUCER & CONSUMER FEATURES
...
curl -X POST http://localhost:8081/api/kafka/consumers/orders-listener/pause

# Check status — should show paused: true
curl http://localhost:8081/api/kafka/consumers/orders-listener/status
...
curl -X POST http://localhost:8081/api/kafka/consumers/orders-listener/resume
...
# Script to check lag and alert if above threshold
check_lag() {
  local group=$1
  local topic=$2
  local threshold=${3:-1000}

  lag=$(docker exec kafka-1 kafka-consumer-groups \
    --bootstrap-server $BROKERS \
    --describe --group "$group" 2>/dev/null | \
    awk -v topic="$topic" '$1==topic {sum+=$6} END {print sum+0}')

  echo "Group: $group | Topic: $topic | Total Lag: $lag"
  if [ "$lag" -gt "$threshold" ]; then
    echo "ALERT: Lag $lag exceeds threshold $threshold — scale up consumers!"
  fi
}

check_lag "order-processor" "orders" 1000
check_lag "payment-processor" "payments" 500
...
docker exec -it kafka-1 kafka-consumer-groups \
  --bootstrap-server $BROKERS --describe --all-groups | grep orders
